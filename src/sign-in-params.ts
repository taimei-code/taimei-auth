...
const serviceNameSchema = z.enum(
  Object.keys(TAIMEI_SERVICES) as [ServiceName, ...ServiceName[]]
);
...
export const signInParamsSchema = z
  ...
  .refine(
    (data) => validateRedirectUrl(data.redirect_url, data.service_name),
    { path: ["redirect_url"], message: "redirect_url is not in allowlist" }
  )
  .refine(
    (data) =>
      data.sign_up_url === undefined ||
      validateRedirectUrl(data.sign_up_url, data.service_name),
