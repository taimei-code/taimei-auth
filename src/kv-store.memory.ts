// Bun (bun test / bun run src/index.ts) 用。単一プロセス前提で、KvStore (DO) と同じ 1 entry モデルを Map に持つ。
...
export class MemoryKvStore {
  ...
  set(key: string, value: string, ttlSec?: number): void {
    this.entries.set(key, { value, expiresAt: ttlSec ? Date.now() + ttlSec * 1000 : null });
  }
  ...
  // Redis TTL と同じ契約: 残秒 (切り上げ)、期限なしは -1、存在しなければ -2。
