...
import { runTest } from "./live-runner";
...
describe("Redis keep-alive", () => {
  test("TTL 付きの SET でキーを書き、値は実行時刻 (ISO 8601) になる", () =>
    runTest("redis-keepalive-")(
      Effect.gen(function* () {
        yield* Effect.promise(() => redisStorage.delete(REDIS_KEEPALIVE_KEY));
        const before = Date.now();

        yield* touchRedisKeepAliveProgram;

        const redis = yield* Effect.promise(() => getRedis());
        const value = yield* Effect.promise(() => redis.get(REDIS_KEEPALIVE_KEY));
        ...
        expect(yield* Effect.promise(() => redis.ttl(REDIS_KEEPALIVE_KEY))).toBeGreaterThan(0);
