...
// リポジトリ全体で使う service の stub (src/CLAUDE.md「test配置」)。domain 固有の stub は各 domain の
// __tests__/test-layers.ts に置く。program 側で `Effect.provide(layer)` して使う。

// 数えられない状態を注入する TTL store。試行枠が fail-closed と fail-open のどちらとして扱われるかを、呼び出し側ごとに観測するための差し替え点。
...
// 固定の count を返す TTL store。上限の境界と第 2 の防御線 (count 0) を、TTL store 無しで観測する。
