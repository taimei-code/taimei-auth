...
// 依存分類 (どの package が runner image に載るか) と docker build 契約は、設定ファイル同士の
// 整合でしか成立していない — biome の ban list / package.json の section / Dockerfile の stage 構成 /
// workflow の action pin。どれかが片側だけ動いても lint も typecheck も build も緑のままなので、
// 設定を text として読んで drift を検出する (web-shared-core-runtime-free.test.ts と同型)。
// 分類規約そのものの正本: docs/adr/0014-docker-runner-dev-stage-separation.md
//
// この test は banned package を一切 import せず設定を読むだけなので、検査対象の override 群と
// 干渉しない。
...
const EXPECTED_WEB_ONLY_COPY_COUNT = 2;
...
const EXPECTED_CONNECT_NODE_COPY_COUNT = 2;
...
const REPRESENTATIVE_SERVER_FILE = "src/handlers/account-company.ts";
const REPRESENTATIVE_DB_FILE = "db/schema.ts";
const REPRESENTATIVE_MANAGEMENT_FILE = "management/disable-user-mfa.ts";
...
function restrictedPatterns(override: BiomeOverride): RestrictedPattern[] {
  return override.linter?.rules?.style?.noRestrictedImports?.options?.patterns ?? [];
}

function webOnlyGroupOf(override: BiomeOverride): string[] | null {
...
function webOnlyGroups(config: BiomeConfig): string[][] {
  const groups: string[][] = [];
...
function overrideMatches(override: BiomeOverride, path: string): boolean {
  let included = false;
...
// 内容で決まる。
function effectiveOverride(config: BiomeConfig, path: string): BiomeOverride | null {
...
function effectiveWebOnlyGroup(config: BiomeConfig, path: string): string[] | null {
  const effective = effectiveOverride(config, path);
  return effective === null ? null : webOnlyGroupOf(effective);
}

function hasConnectNodeBan(override: BiomeOverride): boolean {
...
function webOnlyBanViolations(config: BiomeConfig): string[] {
...
function classificationViolations(diff: ClassificationDiff): string[] {
  const violations: string[] = [];
  const fixHint =
...
function dockerfileViolations(dockerfile: string, workspacePackages: string[]): string[] {
  const violations: string[] = [];
...
function workflowPinViolations(workflow: string, label: string): string[] {
...
describe("依存分類と docker build 契約の config invariant", () => {
