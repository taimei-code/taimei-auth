...
// Workers = Durable Object (1 key = 1 object)、Bun = in-memory。better-auth secondaryStorage の契約 (JSON 文字列) はそのまま。
...
export function toRateWindowResult(rawCount: unknown): RateWindowResult {
  const count = typeof rawCount === "boolean" ? Number.NaN : Number(rawCount);
  if (!Number.isFinite(count) || count < 1) {
    throw new Error(`incrementRateWindow: 応答が契約に外れる (count=${String(rawCount)})`);
  }
  return { count };
}
