...
- 非同期: `Effect.all` / `Effect.timeout` / `Effect.retry`。timeout は境界 service に既定値を置く (Redis 2s、email 10s)。retry は冪等な境界 (Redis 読み取り系) にだけ `src/redis-service.ts` の policy (`withRedisRetry`) で宣言し、再試行 3 回、backoff の合計 1s 未満 (attempt ごとの timeout は別。帰結は Consequences)
...
- `auth.ts` から静的に辿れる module (`auth-plugins/*`、better-auth callback) が `runtime.ts` を import する: `runtime → auth-wiring → auth → plugins → runtime` の環で `AuthApiLive` が TDZ になる。当初は関数内の `await import("./runtime")` で環を隠していたが、2026-09-21 に `initAuth(runtime)` で runtime を注入する形へ変え、`AuthApiLive` を `auth-wiring.ts` に分離して環自体を無くした (import 環は fallow の `circular-dependency` が止める)
...
- `Redis` の retry 対象 (get) は 2s timeout が attempt ごとに掛かる (Decision の非同期項の値)。Redis が応答しない時の worst case は約 8.7s (4 attempt + backoff) で、`/api/mfa/challenge` の読み取りがこの経路にあるため Redis 断のあいだ応答が延びる
