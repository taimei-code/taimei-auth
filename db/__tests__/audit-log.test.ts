...
describe("audit log repository", () => {
  ...
  // MFA の有効化・無効化は、乗っ取り犯が勝手に行った操作を本人が後から辿れる唯一の記録なので、
  // event_type 2 種と payload の形が union に含まれていること自体が前提条件になる。
  test("QA-M-01 mfa_enabled / mfa_disabled が union に載り 1 行ずつ追加される", async () => {
    ...
    // secret、リカバリーコード、残数は payload に含めない (監査ログの閲覧を第二要素の漏洩経路にしない)。
    expect(rows[0]?.payload).toEqual({ ip: "1.2.3.4", userAgent: "test/1.0" });
    // ip が null になり得るのは、リクエストを持たない運用救済スクリプトの経路があるため。
