  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">アクティブなログインセッション一覧</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {sessions?.map((s: SessionItem) => (
            <Card key={s.id}>
              <CardHeader>
                <CardTitle className="text-base">{s.userAgent ?? "Unknown device"}</CardTitle>
                <CardDescription>
                  IP: {s.ipAddress ?? "—"} / 期限: {new Date(s.expiresAt).toLocaleString("ja-JP")}
                </CardDescription>
              </CardHeader>
            </Card>
          ))}
