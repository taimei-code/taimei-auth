#!/usr/bin/env bash
# compose と e2e のためのローカル起動。wrangler dev は process の環境変数を worker の binding にしないため、
# 環境変数を env-file に書き出して渡す。wrangler.jsonc の vars (本番値) は空でも必ず上書きする
# (集合が一致することは src/__tests__/wrangler-dev-env.test.ts が固定する)。
set -euo pipefail
cd "$(dirname "$0")/.."
...
mkdir -p .wrangler
: > "$env_file"
# 値は single quote で囲む (囲まないと dotenv-expand が `$` を展開し、` #` 以降を落とすため)。
...
# --local-upstream が無いと routes の host (auth.taimei-code.com) が Origin に書き換わり、cookie 付きの POST が 403 になる。
