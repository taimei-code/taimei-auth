#!/usr/bin/env bash
# wrangler dev は process の環境変数を worker の binding にしないため env-file で渡す。wrangler.jsonc の vars (本番値) は空でも必ず上書きする (集合の一致は src/__tests__/wrangler-dev-env.test.ts が固定)
set -euo pipefail
cd "$(dirname "$0")/.."
